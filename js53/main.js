...
import {CONFIG} from "./config.js";import {QualityManager} from "./QualityManager.js";import {World} from "./world/World.js?v=78.0";import {Player} from "./player/Player.js";import {Controls} from "./player/Controls.js";import {MobManager} from "./entities/Mob.js?v=78.0";import {MobSpawner} from "./entities/MobSpawner.js";import {Particles} from "./rendering/Particles.js";import {Lighting} from "./rendering/Lighting.js";import {Effects} from "./rendering/Effects.js";import {Weather} from "./rendering/Weather.js";import {AudioManager} from "./audio/AudioManager.js";import {Inventory} from "./inventory/Inventory.js";import {RECIPES_2X2,RECIPES_3X3} from "./inventory/Crafting.js?v=78.0";import {RECIPE_MANAGER} from "./inventory/RecipeManager.js?v=78.0";import {CraftingManager} from "./inventory/CraftingManager.js?v=78.0";import {CraftingTableUI} from "./inventory/CraftingTableUI.js?v=78.0";import {RecipeBookUI} from "./inventory/RecipeBookUI.js?v=78.0";import {HUD} from "./ui/HUD.js";import {Menu} from "./ui/Menu.js";import {SettingsManager} from "./ui/SettingsManager.js";import {SettingsUIController} from "./ui/SettingsUIController.js";import {MainMenuManager} from "./ui/MainMenuManager.js";import {SaveManager} from "./save/SaveManager.js";import {BLOCK,INFO,ITEM,ICON} from "./world/Block.js?v=78.0";import {Chunk} from "./world/Chunk.js";import {MiningSystem} from "./world/MiningSystem.js";import {SurvivalSystems} from "./systems/SurvivalSystems.js";import {NetworkManager} from "./network/NetworkManager.js";
import {WaterPhysicsController} from "./world/WaterPhysicsController.js";import {GIRendererController} from "./rendering/GIRendererController.js?v=78.0";import {VolumetricLightingManager} from "./rendering/VolumetricLightingManager.js?v=78.0";import {DebugConsole} from "./debug/DebugConsole.js";import {InventoryUI} from "./inventory/InventoryUI.js?v=78.0";import {InputTouchFilter} from "./input/InputTouchFilter.js";
class Game{
 constructor(){this.skipUnloadSave=false;const save=SaveManager.loadActive();if(save?.seed)CONFIG.WORLD.SEED=save.seed;this.quality=new QualityManager();CONFIG.QUALITY=this.quality.preset;
  this.scene=new THREE.Scene();this.scene.background=new THREE.Color(0x87c9ef);this.scene.add(new THREE.HemisphereLight(0xffffff,0x4f5f45,1.35));this.scene.add(new THREE.AmbientLight(0xffffff,0.35));const vp0={w:Math.max(1,Math.round(window.visualViewport?.width||window.innerWidth||1)),h:Math.max(1,Math.round(window.visualViewport?.height||window.innerHeight||1))};this.camera=new THREE.PerspectiveCamera(CONFIG.RENDER.FOV,vp0.w/vp0.h,.05,CONFIG.RENDER.FAR);try{this.renderer=new THREE.WebGLRenderer({antialias:false,powerPreference:this.quality.tier==="low"?"low-power":"high-performance",stencil:false,depth:true,preserveDrawingBuffer:false});}catch(e){throw new Error("WebGL недоступен на этом устройстве: "+(e?.message||e))}this.quality.configureRenderer(this.renderer);if(this.quality.tier==="low"||this.quality.tier==="mobile")this.renderer.toneMapping=THREE.NoToneMapping;this.resizeRenderer();this.renderer.setScissorTest(false);this.renderer.outputColorSpace=THREE.SRGBColorSpace;this.renderer.shadowMap.enabled=this.quality.preset.shadows;this.renderer.shadowMap.type=THREE.PCFSoftShadowMap;this.renderer.toneMapping=(this.quality.tier==="low"||this.quality.tier==="mobile")?THREE.NoToneMapping:THREE.ACESFilmicToneMapping;this.renderer.toneMappingExposure=(this.quality.tier==="low"||this.quality.tier==="mobile")?1:1.05;document.getElementById("game").appendChild(this.renderer.domElement);
  this.inputFilter=new InputTouchFilter({dragThreshold:14,cooldownMs:260});this.controls=new Controls(this.camera,this.renderer.domElement,this.inputFilter);this.controls.loadTouchSensitivity();this.world=new World(this.scene,CONFIG);try{this.world.loadChanges(Array.isArray(save?.changes)?save.changes:[])}catch(e){console.warn("Saved world changes ignored",e);try{this.world.loadChanges([])}catch{}}this.player=new Player(this.camera,this.world,this.controls,CONFIG);this.player.game=this;this.inventory=new Inventory();this.player.setInventory(this.inventory);if(!save?.inventory){this.inventory.add(ITEM.SEEDS,8);this.inventory.add(ITEM.HOE,1);this.inventory.add(BLOCK.GRASS,32);this.inventory.add(BLOCK.DIRT,64);this.inventory.add(BLOCK.STONE,64);this.inventory.add(BLOCK.LOG,16);this.inventory.add(BLOCK.PLANKS,32);this.inventory.add(BLOCK.SAND,32);this.inventory.add(ITEM.APPLE,3);for(let i=0;i<9;i++){const src=this.inventory.slots[i];this.inventory.slots[27+i]={...src};this.inventory.durability[27+i]=this.inventory.durability[i];this.inventory.slots[i]={id:0,count:0};this.inventory.durability[i]=0}}
  this.hud=new HUD(this.player,this.inventory,this);this.mobs=new MobManager(this.scene,this.world,CONFIG.QUALITY);this.mobSpawner=new MobSpawner(this.mobs,this.world,CONFIG.QUALITY);this.water=new WaterPhysicsController(this);this.friendlyTimer=0;this.particles=new Particles(this.scene,CONFIG.QUALITY);this.light=null;this.weather=null;this.effects=new Effects(this.renderer);this.gi=new GIRendererController(this.renderer,this.scene,this.camera,this.world,this.quality);this.volumetric=new VolumetricLightingManager(this);this.audio=new AudioManager();this.audio.attachGame(this);this.settings=new SettingsManager(this);this.menu=new Menu(this);this.settingsUI=new SettingsUIController(this,this.settings);this.mainMenuManager=new MainMenuManager(this);this.settings.applyAll();this.systems=new SurvivalSystems(this);this.mining=new MiningSystem(this);this.hud.systems=this.systems;this.network=new NetworkManager(this);this.debug=new DebugConsole(this);this.THREE=THREE;this.vec=(x,y,z)=>new THREE.Vector3(x,y,z);this.running=false;this.inventoryOpen=false;this.mode="singleplayer";this.respawnPoint=null;this.bedRespawnKey=null;this.reducedMotion=false;this.installPrompt=null;this.lastFootstep=0;this.lastPlayerX=0;this.lastPlayerZ=0;this.breakProgress=0;this.miningHeld=false;this.breakTargetKey="";this.breakTargetId=0;this.breakTime=0;this.perfFrames=0;this.perfTime=0;this.perfCooldown=0;this.perfStable=0;this.dynamicPixelRatio=this.quality.preset.pixelRatio;this.outline=this.makeOutline();this.time=save?.time||180;this.last=performance.now();this.mobTimer=0;this.saveTimer=0;this.mineTimer=0;this.worldStreamTimer=0;this.fallTimer=0;this.smokeTimer=0;this.raycastTimer=0;this.cachedTarget=null;this.chunkTickTimer=0;this.meshFrameSkip=0;const qv=document.getElementById("qualityValue"),qs=document.getElementById("qualitySelect");if(qv)qv.textContent=this.quality.tier.toUpperCase();if(qs)qs.value=this.quality.tier;this.setupInput();this.setupTouchLayout();this.setupInventory();this.setupInstall();if(save?.inventory){try{this.inventory.deserialize(save.inventory);this.inventoryUI?.render()}catch(e){console.warn("Inventory save ignored",e)}}if(save?.systems){try{this.systems.deserialize(save.systems)}catch(e){console.warn("Old systems save ignored",e)}}if(save?.respawnPoint){this.respawnPoint=save.respawnPoint;this.bedRespawnKey=save.bedRespawnKey||null}if(save?.player){this.player.xp=Number(save.player.xp||0);this.player.level=Math.max(1,Number(save.player.level||1));this.player.stamina=Math.max(0,Math.min(100,Number(save.player.stamina??100)));const x=Number(save.player.x),y=Number(save.player.y),z=Number(save.player.z);if([x,y,z].every(Number.isFinite)){this.player.pos.set(x,Math.max(1,Math.min(CONFIG.WORLD.HEIGHT-2,y)),z)}}this.bindViewportResize();this.resize();this.setModeBadge();this.setGameUI(false);this.loop()}
 setupAudioSettings(){
  // Legacy compatibility: audio controls are now owned by SettingsUIController.
  // Keep this method intentionally side-effect free so old integrations cannot break boot.
  return true;
}
 setGameUI(visible){
 ...
 async start(){
  ...
  try{
    ...
    // Always re-seat the player on the generated terrain at world load.
    // Old saves can contain a stale Y coordinate from an earlier terrain build.
    const groundY=this.findGround(this.player.pos.x,this.player.pos.z);
    const savedY=Number(this.player.pos.y);
    if(!Number.isFinite(savedY) || savedY<1 || Math.abs(savedY-groundY)>6){
      this.player.pos.y=groundY+.02;
      this.player.vel.set(0,0,0);
    }
    this.camera.position.set(this.player.pos.x,this.player.pos.y+1.62,this.player.pos.z);
    // Start with a clear downward view so the terrain is immediately visible.
    this.controls.pitch=-0.48;
    ...
    this.running=true;
    this.setGameUI(true);
    // Always show a small passive herd near a fresh/empty spawn; later the natural spawner takes over.
    if(this.mobs.mobs.filter(m=>['cow','pig','sheep','chicken'].includes(m.type)).length<3)this.spawnStarterCreatures();
    ...
    requestAnimationFrame(()=>requestAnimationFrame(()=>{
      try{this.light=new Lighting(this.scene,this.world,CONFIG);this.volumetric.applyPreset(this.quality.tier)}catch(e){console.warn("Lighting disabled",e);this.light=null}
      ...
      requestAnimationFrame(async()=>{try{await this.world.generateAround(this.player.pos.x,this.player.pos.z);this.world.ensureNearbyVillage?.(this.player.pos.x,this.player.pos.z);this.spawnStarterCreatures();this.mobSpawner?.spawnVillagersNear(this.player);setTimeout(()=>{try{this.world.ensureNearbyVillage?.(this.player.pos.x,this.player.pos.z);this.spawnStarterCreatures();this.mobSpawner?.spawnVillagersNear(this.player);}catch(e){console.warn("Nearby content retry skipped",e)}},1400);}catch(e){console.warn("Nearby village setup skipped",e)}});
    }));
 ...
 spawnStarterCreatures(){
  const passive=["cow","pig","sheep","chicken"];
  if(this.mobs.mobs.filter(m=>passive.includes(m.type)).length>=4)return;
  const placed=[];
  const tryPlace=(x,z,type)=>{
    const xi=Math.floor(x),zi=Math.floor(z);
    const s=this.world.cfg.WORLD.CHUNK_SIZE;
    if(!this.world.chunks.has(this.world.key(Math.floor(xi/s),Math.floor(zi/s))))return false;
    for(let y=this.world.cfg.WORLD.HEIGHT-2;y>=2;y--){
      const ground=this.world.getBlock(xi,y,zi);
      const above=this.world.getBlock(xi,y+1,zi);
      const above2=this.world.getBlock(xi,y+2,zi);
      if(ground===BLOCK.GRASS && !INFO[above]?.solid && !INFO[above]?.liquid && !INFO[above2]?.solid && !INFO[above2]?.liquid){
        const m=this.mobs.spawnFriendly(xi+.5,y+1,zi+.5,type);
        return !!m;
      }
    }
    return false;
  };
  const offsets=[[4,-6],[-4,-6],[7,-2],[-7,-2],[5,5],[-5,5],[9,4],[-9,4],[0,-9],[0,9]];
  for(let i=0;i<offsets.length&&placed.length<4;i++){
    const [dx,dz]=offsets[i];
    const type=passive[i%passive.length];
    if(tryPlace(this.player.pos.x+dx,this.player.pos.z+dz,type))placed.push(type);
  }
  if(placed.length<4){
    const s=this.world.cfg.WORLD.CHUNK_SIZE,pcx=Math.floor(this.player.pos.x/s),pcz=Math.floor(this.player.pos.z/s);
    for(let x=pcx*s+1;x<pcx*s+s-1&&placed.length<4;x++)for(let z=pcz*s+1;z<pcz*s+s-1&&placed.length<4;z++){
      const type=passive[placed.length%passive.length];
      if(tryPlace(x+.5,z+.5,type))placed.push(type);
    }
  }
 ...
 navigateToWorld(id){if(!id)return false;this.skipUnloadSave=true;try{localStorage.setItem("vs_pending_launch_v78",String(id))}catch{}try{sessionStorage.setItem("vs_pending_launch_v78",String(id))}catch{}try{const u=new URL(location.href);u.searchParams.set("launch",String(id));location.replace(u.href);return true}catch(e){console.error("WORLD NAVIGATION FAILED",e);try{location.reload()}catch{}return true}}
 ...
 setupInventory(){this.craftGrid2=Array.from({length:4},()=>({id:0,count:0}));this.craftGrid3=Array.from({length:9},()=>({id:0,count:0}));this.craftMode="player";try{this.craftingManager=new CraftingManager(this,RECIPE_MANAGER);this.inventoryUI=new InventoryUI(this);this.recipeBookUI=new RecipeBookUI(this,this.craftingManager);this.recipeBookUI.setup();this.craftingTableUI=new CraftingTableUI(this,this.inventoryUI);this.inventoryUI.setupModeButtons();this.renderInventory()}catch(e){console.error("Inventory UI init failed; gameplay will continue",e);this.inventoryUI=null}}
 openCraftingTable(){this.craftingTableUI?.open();if(!this.craftingTableUI){this.craftMode="table";this.inventoryUI?.open();this.inventoryOpen=true;this.inventoryUI?.render()}}
 returnCraftGrid(grid){this.craftingManager?.returnGridToInventory(grid);return true}
 closeCraftingUI(){if(this.craftMode==="table"){this.craftingTableUI?.close();}else{this.craftingManager?.returnGridToInventory(this.craftGrid2);this.inventoryUI?.close();}this.craftMode="player";this.inventoryOpen=false;document.getElementById("inventoryPanel")?.classList.add("hidden")}
 renderInventory(){this.inventoryUI?.render()}
 setupCraftingInventoryInteractions(){this.inventoryUI?.render()}
 getCurrentRecipes(){return this.craftingManager?this.craftingManager.recipes():(this.craftMode==="table"?RECIPES_3X3:RECIPES_2X2)}
 ...
 setupInput(){document.getElementById("closeMultiplayer")?.addEventListener("click",()=>document.getElementById("multiplayerPanel")?.classList.add("hidden"));document.getElementById("connectButton")?.addEventListener("click",()=>{const v=document.getElementById("serverAddress");let u=v?.value.trim();if(!u)u=`${location.protocol==="https:"?"wss":"ws"}://${location.host}/ws`;this.mode="multiplayer";this.setModeBadge();this.network.connect(u)});document.getElementById("disconnectButton")?.addEventListener("click",()=>this.network.disconnect());document.getElementById("chatSend")?.addEventListener("click",()=>{const i=document.getElementById("chatInput");this.network.chat(i.value);i.value=""});document.getElementById("chatInput")?.addEventListener("keydown",e=>{if(e.key==="Enter"){this.network.chat(e.target.value);e.target.value=""}});addEventListener("keydown",e=>{if(e.code==="Escape"){if(this.inventoryOpen){this.toggleInventory();return}if(this.running){this.running=false;this.menu.showPause();document.exitPointerLock?.()}return}if(e.code==="KeyE"||e.code==="KeyI"){this.toggleInventory();return}if(e.code.startsWith("Digit")){const n=+e.code.slice(5)-1;if(n>=0&&n<9)this.inventory.selected=n}if(e.code==="KeyF"&&this.player.eat()){this.audio.eat?.();setTimeout(()=>this.audio.finishEat?.(),260);this.particles.burst(this.player.pos.clone().add(new THREE.Vector3(0,1,0)),0xff5533,8)}});
  addEventListener("mousedown",e=>{if(!this.running)return;this.inputFilter?.begin(e,"mouse");if(e.button===0){this.miningHeld=true;this.attackOrBreak()}if(e.button===2)this.placeBlock()});addEventListener("mouseup",e=>{if(e.button===0)this.stopMining();this.inputFilter?.end(e)});addEventListener("blur",()=>this.stopMining());addEventListener("contextmenu",e=>e.preventDefault());addEventListener("wheel",e=>{this.inventory.selected=(this.inventory.selected+(e.deltaY>0?1:-1)+9)%9});const bindTouchAction=(id,fn)=>{const el=document.getElementById(id);if(!el)return;el.addEventListener("pointerdown",e=>{if(this.editingControls||!this.running)return;e.preventDefault();e.stopPropagation();el.setPointerCapture?.(e.pointerId);fn()},{passive:false})};bindTouchAction("jumpButton",()=>{this.player.jump()});bindTouchAction("attackButton",()=>{this.miningHeld=true;this.attackOrBreak()});bindTouchAction("useButton",()=>this.useSelected());bindTouchAction("inventoryButton",()=>this.toggleInventory());
  ...
  canvas.addEventListener("pointerdown",e=>{if(this.editingControls||!this.running||e.pointerType!=="touch"||uiTouch(e))return;this.inputFilter?.begin(e,"world");worldTouch={id:e.pointerId,x:e.clientX,y:e.clientY,moved:false,started:false,timer:null};try{canvas.setPointerCapture(e.pointerId)}catch{};worldTouch.timer=setTimeout(()=>{if(worldTouch?.id===e.pointerId&&!worldTouch.moved&&this.running&&this.inputFilter?.canInteract()){if(this.touchActionMode==="mine"){this.miningHeld=true;this.attackOrBreak();worldTouch.started=true}}},120);e.preventDefault()},{passive:false});
  canvas.addEventListener("pointermove",e=>{if(!worldTouch||worldTouch.id!==e.pointerId)return;const moved=this.inputFilter?.move(e,"world")??(Math.hypot(e.clientX-worldTouch.x,e.clientY-worldTouch.y)>14);if(moved){worldTouch.moved=true;if(worldTouch.timer){clearTimeout(worldTouch.timer);worldTouch.timer=null}if(this.touchActionMode==="mine")this.stopMining();e.preventDefault()}},{passive:false});
  const endWorldTouch=e=>{if(!worldTouch||worldTouch.id!==e.pointerId)return;const w=worldTouch;if(w.timer)clearTimeout(w.timer);const result=this.inputFilter?.end(e,"world")||{tap:!w.moved,drag:w.moved};const tap=result.tap&&this.inputFilter?.canInteract();const mode=this.touchActionMode;worldTouch=null;if(mode==="place"&&tap&&this.running)this.placeBlock();else if(mode==="mine"&&!w.started&&tap&&this.running){this.miningHeld=true;this.attackOrBreak();this.stopMining()}else if(mode==="mine")this.stopMining();try{canvas.releasePointerCapture?.(e.pointerId)}catch{}};
 ...
 useSelected(){const s=this.inventory.selectedItem(),info=s?.id?INFO[s.id]:null;if(info?.food){if(this.player.eat()){this.audio.eat?.();setTimeout(()=>this.audio.finishEat?.(),260)}return}if(s?.id===ITEM.HOE){const hit=this.player.raycast();if(hit&&[BLOCK.DIRT,BLOCK.GRASS].includes(hit.id)&&this.world.getBlock(hit.block.x,hit.block.y+1,hit.block.z)===BLOCK.AIR){this.world.setBlock(hit.block.x,hit.block.y,hit.block.z,BLOCK.FARMLAND);this.audio.block();this.particles.dust(new THREE.Vector3(hit.block.x+.5,hit.block.y+.5,hit.block.z+.5),0x7a4c2b,8);this.systems.award(2,"Первая грядка")}return}if(info?.place||info?.solid)this.placeBlock()}
  attackOrBreak(){const target=this.player.raycast();if(this.systems.interact(target))return;if(this.mobs.shear(this.player)){this.audio.pickup?.();this.systems.award(5,"Стрижка овцы");return;}const result=this.mobs.attack(this.player);if(result.hit){this.audio.hit();this.audio.mobHurt?.(result.mob?.type,result.mob?.pos);this.effects.hit();this.stopMining();if(result.dead){if(result.drops){for(const [id,count] of result.drops)this.inventory.add(id,count)}else{this.inventory.add(ITEM.RAW_MEAT,1);if(Math.random()<.25)this.inventory.add(ITEM.APPLE,1)}this.systems.award(20,result.mob?.type?"Охотник на животных":"Охотник на нежить");this.audio.mobDeath?.(result.mob?.type,result.mob?.pos);this.particles.burst(this.player.pos.clone().add(new THREE.Vector3(0,1,0)),0xb84b4b,20);}return}this.beginMining(target)}
 ...
 placeBlock(){const hit=this.player.raycast();if(!hit?.previous)return;const p=hit.previous,s=this.inventory.selectedItem(),id=s?.id,info=INFO[id];if(!id||(!info?.solid&&!info?.place))return;let blockId=info.place||id;if(id===ITEM.SEEDS){if(this.world.getBlock(p.x,p.y-1,p.z)!==BLOCK.FARMLAND&&this.world.getBlock(p.x,p.y-1,p.z)!==BLOCK.DIRT)return;blockId=BLOCK.WHEAT}if(this.player.collides({x:p.x+.5,y:p.y,z:p.z+.5}))return;if(!this.inventory.remove(id,1))return;if(!this.world.setBlock(p.x,p.y,p.z,blockId)){this.inventory.add(id,1);return}this.network.blockChanged(p.x,p.y,p.z,blockId);this.audio.blockPlace(blockId,new THREE.Vector3(p.x+.5,p.y+.5,p.z+.5));this.systems.placed(blockId,p);this.systems.award(1,blockId===BLOCK.CHEST?"Строитель":"");this.particles.burst(new THREE.Vector3(p.x+.5,p.y+.5,p.z+.5),0xffffff,5);if(this.light)this.light.lastScan=""}
 loop(){requestAnimationFrame(()=>this.loop());const now=performance.now(),dt=Math.min(.05,(now-this.last)/1000);this.last=now;if(this.settings?.get('maxFPS')>0){const minFrame=1000/this.settings.get('maxFPS');if(now-(this._lastRenderAt||0)<minFrame)return;this._lastRenderAt=now;}this.debug?.update(dt);this.perfFrames++;this.perfTime+=dt;if(this.perfTime>2){const fps=this.perfFrames/this.perfTime;this.perfFrames=0;this.perfTime=0;this.perfCooldown=Math.max(0,this.perfCooldown-2);if(fps<30&&this.perfCooldown<=0){this.perfCooldown=8;this.dynamicPixelRatio=Math.max(.54,this.dynamicPixelRatio-.08);this.renderer.setPixelRatio(Math.min(devicePixelRatio||1,this.dynamicPixelRatio))}else if(fps>55){this.perfStable+=2;if(this.perfStable>=12){this.perfStable=0;this.dynamicPixelRatio=Math.min(this.quality.preset.pixelRatio,this.dynamicPixelRatio+.03);this.renderer.setPixelRatio(Math.min(devicePixelRatio||1,this.dynamicPixelRatio))}}else this.perfStable=0}this.effects.update(dt);this.mineTimer=Math.max(0,this.mineTimer-dt);this.raycastTimer-=dt;if(this.raycastTimer<=0){this.raycastTimer=.055;this.cachedTarget=this.player.raycast()}const target=this.cachedTarget;if(this.running&&!this.inventoryOpen){this.updateMining(dt);this.time+=dt;this.controls.updateCamera();this.water.tick(dt);const prevY=this.player.pos.y,prevVy=this.player.vel.y;this.player.move(dt);this.audio._fallSpeed=Math.max(0,-prevVy);this.audio.update(dt,this);const moved=Math.hypot(this.player.pos.x-this.lastPlayerX,this.player.pos.z-this.lastPlayerZ);if(moved>.001)this.audio.step();this.lastPlayerX=this.player.pos.x;this.lastPlayerZ=this.player.pos.z;const shake=this.reducedMotion?0:this.effects.consumeShake();if(shake){this.camera.position.x+=(Math.random()-.5)*shake;this.camera.position.y+=(Math.random()-.5)*shake}this.mobTimer+=dt;this.saveTimer+=dt;this.mobSpawner?.update(dt,this.player,this.time);const phase=(this.time%CONFIG.DAY.LENGTH)/CONFIG.DAY.LENGTH,night=phase<.25||phase>.75;if(this.mobTimer>3){this.mobTimer=0;if(night&&this.mobs.mobs.length<this.mobs.max){const a=Math.random()*Math.PI*2,r=24+Math.random()*42,x=this.player.pos.x+Math.cos(a)*r,z=this.player.pos.z+Math.sin(a)*r,y=this.findGround(x,z);this.mobs.spawn(x,y,z)}}const oldHealth=this.player.health;if(this.mobs.update(dt,this.player)&&this.player.health<oldHealth){this.effects.damage();this.audio.hurt()}if(this.player.health<=0)this.handleDeath();this.fallTimer+=dt;this.smokeTimer+=dt;this.worldStreamTimer+=dt;if(this.fallTimer>.9){this.fallTimer=0;this.world.tickFalling(this.player)}if(this.smokeTimer>.3){this.smokeTimer=0;this.emitCampfireSmoke()}this.particles.update(dt);this.mining.updateDrops(dt);this.systems.tick(dt);if(this.light)this.light.update(this.time/CONFIG.DAY.LENGTH*Math.PI*2,this.player,dt);this.network.tick(dt);this.network.update(dt);if(this.weather)this.weather.update(dt,this.player,this.time);if(target&&this.world.meshes.size>0){this.outline.visible=true;this.outline.position.set(target.block.x+.5,target.block.y+.5,target.block.z+.5);const op=this.outline.material;op.opacity=.42+this.breakProgress*.48;this.outline.scale.setScalar(1+this.breakProgress*.035)}else{this.outline.visible=false;this.outline.scale.setScalar(1)}if(this.saveTimer>25){this.saveTimer=0;this.save();if(this.network.hosting)this.network.syncHostWorld()}if(this.worldStreamTimer>(this.quality.mobile?2.2:(this.quality.tier==='low'?1.4:.9))){this.worldStreamTimer=0;this.world.generateAround(this.player.pos.x,this.player.pos.z)}}this.hud.update(target,this.time,dt);this.volumetric?.update(dt,(this.time%CONFIG.DAY.LENGTH)/CONFIG.DAY.LENGTH*Math.PI*2,this.player);// The direct renderer is the authoritative game path. Optional GI must never
// replace the final frame on mobile/WebGL; it can be layered in later.
this.renderer.setRenderTarget(null);
this.renderer.setScissorTest(false);
const rs=this.renderer.getSize(new THREE.Vector2());
this.renderer.setViewport(0,0,rs.x,rs.y);
this.renderer.render(this.scene,this.camera);if(this.world.meshQueue.length){this.meshFrameSkip=(this.meshFrameSkip+1)%3;if(!this.quality.mobile||this.meshFrameSkip===0){this.world.processMeshQueue(this.quality.mobile?.38:(this.quality.tier==='low'?1.25:this.quality.tier==='medium'?2:2.8))}}if(this.world.meshes.size>=1&&this.world.starterVisual)this.world.removeStarterVisual();if(this.running&&this.world.chunks.size>0&&this.world.meshes.size===0&&this.world.meshQueue.length===0){for(const c of this.world.chunks.values())this.world.queueRebuild(c)}}
 ...
 setupInstall(){window.addEventListener("beforeinstallprompt",e=>{e.preventDefault();this.installPrompt=e;const b=document.getElementById("nativeInstallButton");if(b)b.textContent="⬇ УСТАНОВИТЬ ПРИЛОЖЕНИЕ"});window.addEventListener("appinstalled",()=>{this.installPrompt=null;const b=document.getElementById("nativeInstallButton");if(b)b.textContent="✓ ПРИЛОЖЕНИЕ УСТАНОВЛЕНО"});document.addEventListener("fullscreenchange",()=>{const active=!!document.fullscreenElement;for(const id of ["fullscreenButton","fullscreenPauseButton"]){const b=document.getElementById(id);if(b)b.textContent=active?"⛶ ВЫЙТИ ИЗ ПОЛНОГО ЭКРАНА":"⛶ ПОЛНЫЙ ЭКРАН"}})}
...
async toggleFullscreen(){try{if(document.fullscreenElement){await document.exitFullscreen();return}const el=document.documentElement;if(el.requestFullscreen)await el.requestFullscreen({navigationUI:"hide"});else{const ios=window.navigator.standalone||/iphone|ipad|ipod/i.test(navigator.userAgent);if(ios){const t=document.getElementById("saveToast");t.textContent="Для полноэкранного режима добавь игру на экран Домой";t.classList.add("show");setTimeout(()=>t.classList.remove("show"),2200)}}}catch(e){}}
 ...
 setQuality(tier,persist=true){if(!['mobile','low','medium','high','ultra'].includes(tier))return;this.quality.choose(tier,persist);CONFIG.QUALITY=this.quality.preset;this.renderer.setPixelRatio(Math.min(devicePixelRatio||1,this.quality.preset.pixelRatio));this.renderer.toneMapping=(this.quality.tier==="low"||this.quality.tier==="mobile")?THREE.NoToneMapping:THREE.ACESFilmicToneMapping;this.renderer.shadowMap.enabled=this.quality.preset.shadows;if(this.light)this.light.maxLights=this.quality.preset.maxLights;if(this.gi)this.gi.setPreset(tier);if(this.volumetric)this.volumetric.applyPreset(tier);this.particles.maxItems=this.quality.preset.particles;this.world.cfg.WORLD.RENDER_DISTANCE=this.quality.preset.renderDistance;this.world.lastCenter="";const v=document.getElementById("qualityValue");if(v)v.textContent=this.quality.tier.toUpperCase()}
 ...
 resizeRenderer(){const {w,h}=this.getViewportSize();this.camera.aspect=w/h;this.camera.updateProjectionMatrix();this.renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,this.dynamicPixelRatio||this.quality?.preset?.pixelRatio||1));this.renderer.setSize(w,h,false);this.renderer.setViewport(0,0,w,h);this.renderer.setScissorTest(false);this.world?.pbr?.setViewport(w*(this.renderer.getPixelRatio?.()||1),h*(this.renderer.getPixelRatio?.()||1));this.gi?.resize();const c=this.renderer.domElement;c.style.position="absolute";c.style.left="0";c.style.top="0";c.style.display="block";c.style.width="100%";c.style.height="100%";c.style.maxWidth="none";c.style.maxHeight="none"}
...
try{
  ...
  if(!id){for(const key of ["vs_pending_launch_v77_7","vs_pending_launch_v77_6","vs_pending_launch_v77"]){try{id=localStorage.getItem(key);if(id)localStorage.removeItem(key);if(id)break}catch{}}}
  if(!id){for(const key of ["vs_pending_launch_v77_7","vs_pending_launch_v77_6","vs_pending_launch_v77"]){try{id=sessionStorage.getItem(key);if(id)sessionStorage.removeItem(key);if(id)break}catch{}}}
