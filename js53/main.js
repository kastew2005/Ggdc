...
class Game{
 ...
 navigateToWorld(id){if(!id)return false;this.skipUnloadSave=true;try{localStorage.setItem("vs_pending_launch_v77",String(id))}catch{}try{sessionStorage.setItem("vs_pending_launch_v77",String(id))}catch{}try{const u=new URL(location.href);u.searchParams.set("launch",String(id));location.replace(u.href);return true}catch(e){console.error("WORLD NAVIGATION FAILED",e);try{location.reload()}catch{}return true}}
...
try{
  ...
  if(!id){try{id=localStorage.getItem("vs_pending_launch_v77");if(id)localStorage.removeItem("vs_pending_launch_v77")}catch{}}
  if(!id){try{id=sessionStorage.getItem("vs_pending_launch_v77");if(id)sessionStorage.removeItem("vs_pending_launch_v77")}catch{}}
