import {SaveManager} from "../save/SaveManager.js";import {PixelButton} from "./PixelButton.js";
export class Menu{
 ...
 rootButtons(){this.main?.querySelectorAll('button').forEach(b=>new PixelButton(b,{audio:this.game.audio}))}
 ...
 createWorld(){const name=document.getElementById("newWorldName")?.value.trim()||`Новый мир ${SaveManager.listWorlds().length+1}`;const seed=document.getElementById("newWorldSeed")?.value.trim()||"";this.game.newWorld(name,seed);return true}
