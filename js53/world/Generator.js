...
import {CaveGenerator} from './CaveGenerator.js?v=76.6';
import {StructureGenerator} from './StructureGenerator.js?v=76.6';
...
export class BiomeGenerator {
  ...
  treeAllowed(x,z,b){if(!['plains','birch_grove','mixed_forest','jungle'].includes(b))return false;const r=this.random(x,z,900),candidate=b==='jungle'?.085:b==='birch_grove'?.12:b==='mixed_forest'?.08:.018;if(r>=candidate)return false;for(let dx=-2;dx<=2;dx++)for(let dz=-2;dz<=2;dz++){if(!dx&&!dz)continue;const nb=this.biome(x+dx,z+dz);const nr=this.random(x+dx,z+dz,900),nc=nb==='jungle'?.085:nb==='birch_grove'?.12:nb==='mixed_forest'?.08:.018;if(nr<nc&&nr<r)return false}return true}
  ...
  tree(x,y,z,api,type='oak'){const spec={oak:[BLOCK.LOG,BLOCK.LEAVES,5],birch:[BLOCK.BIRCH_LOG,BLOCK.BIRCH_LEAVES,5],spruce:[BLOCK.SPRUCE_LOG,BLOCK.SPRUCE_LEAVES,7],jungle:[BLOCK.JUNGLE_LOG,BLOCK.JUNGLE_LEAVES,8]}[type]||[BLOCK.LOG,BLOCK.LEAVES,5];const[log,leaf,h]=spec;for(let i=0;i<h;i++)api.setBlock(x,y+i,z,log);const radius=type==='spruce'?2:3;for(let dy=1;dy<=3;dy++)for(let dx=-radius;dx<=radius;dx++)for(let dz=-radius;dz<=radius;dz++){const dist=Math.abs(dx)+Math.abs(dz)+(dy===1?1:0);if(dist<=radius+1&&api.getBlock?.(x+dx,y+h-4+dy,z+dz)===BLOCK.AIR)api.setBlock(x+dx,y+h-4+dy,z+dz,leaf)}if(type==='jungle')for(let i=1;i<h-1;i++)if(this.random(x+i,z-i,970)<.38)api.setBlock(x+1,y+i,z,BLOCK.VINE)}
