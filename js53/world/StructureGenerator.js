...
export class StructureGenerator {
  ...
  flat(cx,cz,maxDelta=4){
    const gx=cx*16+8,gz=cz*16+8,h=this.gen.height(gx,gz),b=this.gen.biome(gx,gz,h);
    if(!['plains','desert','birch_grove','mixed_forest'].includes(b))return false;
  ...
  villageAt(cx,cz,force=false){
    const forced=force||this.forced.has(`${cx},${cz}`);
    if(!this.flat(cx,cz,forced?8:4)||(!forced&&this.hash(cx,cz,0)>.38))return null;
    const gx=cx*16+8,gz=cz*16+8,desert=this.gen.biome(gx,gz)==='desert';
    const seed=this.hash(cx,cz,2),houses=4+(seed*3|0),out=[];const villageBase=this.gen.height(gx,gz)+1;
    const add=(x,y,z,id)=>out.push([x,y,z,id]);
    const ground=()=>villageBase;
    ...
    const house=(x,z,w=6,d=6,kind='home')=>{
      ...
      // Chimney and roof ridge details make the roof read from a distance.
      if(kind==='forge'){
        const cx=x+w-2, cz=z+d-2;
        add(cx,y+4,cz,BLOCK.COBBLE);add(cx,y+5,cz,BLOCK.COBBLE);
      }
    ...
    // Keep every building inside the 16x16 village chunk. Older layouts
    // crossed chunk borders, so mobile render distance could show half a house.
    const positions=[[gx-7,gz-7],[gx+2,gz-7],[gx-7,gz+2],[gx+2,gz+2]];
    const count=Math.min(4,Math.max(3,houses));
    for(let i=0;i<count;i++)house(positions[i][0],positions[i][1],5,5,i===3?'forge':'home');
    const pathY=ground(gx,gz)-1;
    for(let x=gx-7;x<=gx+7;x++)for(let z=gz-1;z<=gz+1;z++)add(x,pathY,z,desert?BLOCK.SAND:BLOCK.GRAVEL);
    for(let z=gz-7;z<=gz+7;z++)for(let x=gx-1;x<=gx+1;x++)add(x,pathY,z,desert?BLOCK.SAND:BLOCK.GRAVEL);
    // central well / plaza
    cube(gx-1,pathY+1,gz-1,3,1,3,BLOCK.COBBLE);add(gx,pathY+1,gz,BLOCK.WATER_L4);
    // four lantern posts around the plaza
    for(const [px,pz] of [[gx-6,gz-6],[gx+6,gz-6],[gx-6,gz+6],[gx+6,gz+6]]){add(px,pathY,pz,BLOCK.LOG);add(px,pathY+1,pz,BLOCK.LOG);add(px,pathY+2,pz,BLOCK.LANTERN);}
    // farm
  ...
  forceVillage(cx,cz){if(this.flat(cx,cz,8))this.forced.add(`${cx},${cz}`);return this.villageAt(cx,cz,true)}
